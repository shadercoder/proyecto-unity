...
var octavas					: float		= 4;					//Octavas para la funcion de ruido fBm
var lacunaridad				: float		= 2;					//La lacunaridad (cuanto se desplazan las coordenadas en sucesivas "octavas")
var ganancia				: float		= 0.5;					//El peso que se le da a cada nueva octava
var offset 					: float		= 0.75;					//Valor para determinados algoritmos de ruido
var escala					: float		= 0.09;					//Lo mismo
var agua					: float		= 0.3;					//Entre 0 y 1, en que punto aparece agua
...
private var semillaCoord	: Vector2;							//Numeros aleatorios para la generacion del terreno. 
...
function ruido_fBm(coords : Vector2, nOctavas : int, lacunarity : float, gain : float) {
	...
	for (var i : int = 0; i < nOctavas; i++) {
		ruidoTotal += amplitud * Mathf.PerlinNoise(coords.x * semillaCoord.x, coords.y * semillaCoord.y);
...
function creacionInicial() {
	...
	semillaCoord = Random.insideUnitCircle;
	var fractal : FractalNoise = new FractalNoise(ganancia, lacunaridad, octavas, new Perlin());
	var media : float = 0;
...
//					texturaNorm.SetPixel(j*relTexTabAncho + b, i*relTexTabAlto + a, Color(valor, valor, valor, 1));
				}
		}
	}
	media /= pixels.Length;
	//Poner agua en el mundo a partir del nivel definido en el editor
